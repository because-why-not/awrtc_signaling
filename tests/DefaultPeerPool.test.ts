...
class MockSignalingPeer implements ISignalingPeer {
  ...
  private acceptedOutgoingConnections: Array<{
    peer: ISignalingPeer;
    id: ConnectionId;
  }> = [];
  ...
  acceptOutgoingConnection(
    ...
    this.acceptedOutgoingConnections.push({
      peer: otherPeer,
      id: newConnectionId,
    });
  }
  ...
  // Test helper methods
  getAcceptedOutgoingConnections() {
    return [...this.acceptedOutgoingConnections];
  }
  getAcceptedIncomingConnections() {
    return [...this.acceptedIncomingConnections];
  }
  getDeniedConnections() {
    return [...this.deniedConnections];
  }
  getAcceptedListening() {
    return [...this.acceptedListening];
  }
  getDeniedListening() {
    return [...this.deniedListening];
  }
...
describe("DefaultPeerPool", () => {
  ...
  describe("Address Management", () => {
    ...
    test("should support multiple peers on same address with address sharing", () => {
      ...
      const poolWithSharing = new DefaultPeerPool(
        configWithSharing,
        TestHelper.logger,
      );
  ...
  describe("Connection Requests", () => {
    test("should connect to single peer listening on address", () => {
      ...
      expect(clientPeer.getAcceptedOutgoingConnections()[0].peer).toBe(
        serverPeer,
      );
      expect(clientPeer.getAcceptedOutgoingConnections()[0].id).toEqual(
        connectionId,
      );
    });
  ...
  describe("Address Sharing Flag", () => {
    ...
    test("should handle undefined address_sharing in config", () => {
      ...
      const poolNoSharing = new DefaultPeerPool(
        configNoSharing,
        TestHelper.logger,
      );
