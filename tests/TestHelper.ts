...
export class MockPeerController implements IPeerController {
  private listeningRequests: Array<{ client: SignalingPeer; address: string }> =
    [];
  private stopListeningRequests: Array<{
    client: SignalingPeer;
    address: string;
  }> = [];
  private connectionRequests: Array<{
    client: SignalingPeer;
    address: string;
    id: ConnectionId;
  }> = [];
  ...
  getListeningRequests() {
    return [...this.listeningRequests];
  }
  getStopListeningRequests() {
    return [...this.stopListeningRequests];
  }
