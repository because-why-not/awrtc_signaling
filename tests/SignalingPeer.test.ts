...
describe('SignalingPeer', () => {
    ...
    describe('Peer Connect and Disconnect ', () => {
        ...
        test('should disconnect on connected peer dispose', () => {
            ...
            expect(sentEvents.some(evt => evt.Type === NetEventType.Disconnected)).toBe(true);
            expect(otherSentEvents.some(evt => evt.Type === NetEventType.Disconnected)).toBe(false);
        });
    });
    describe('Message forwarding', () => {
        test('should handle ReliableMessageReceived event and forward to connected peer', () => {
            ...
            const messageData = new Uint8Array([0xFF, 0xFE, 0x68, 0x00, 0x65, 0x00, 0x6C, 0x00, 0x6C, 0x00, 0x6F, 0x00, 0x20, 0x00, 0x77, 0x00, 0x6F, 0x00, 0x72, 0x00, 0x6C, 0x00, 0x64, 0x00]);
        ...
        test('should handle UnreliableMessageReceived event and forward to connected peer', () => {
            ...
            expect(sentEvents).toHaveLength(0); // Original peer shouldn't receive its own message back
            expect(otherSentEvents).toHaveLength(1); // Other peer should receive the forwarded message
            expect(otherSentEvents[0].Type).toBe(NetEventType.UnreliableMessageReceived);
            expect(otherSentEvents[0].MessageData).toEqual(messageData);
        });
        ...
        test('should ignore messages sent to unknown connection ID without crashing', () => {
            ...
            const messageData = new Uint8Array([0xFF, 0xFE, 0x74, 0x00, 0x65, 0x00, 0x73, 0x00, 0x74, 0x00, 0x20, 0x00, 0x6D, 0x00, 0x65, 0x00, 0x73, 0x00, 0x73, 0x00, 0x61, 0x00, 0x67, 0x00, 0x65, 0x00]);
            ...
            expect(() => {
                mockProtocol.simulateNetworkEvent(
                    new NetworkEvent(NetEventType.ReliableMessageReceived, unknownConnectionId, messageData)
                );
            }).not.toThrow();

            expect(() => {
                mockProtocol.simulateNetworkEvent(
                    new NetworkEvent(NetEventType.UnreliableMessageReceived, unknownConnectionId, messageData)
                );
            }).not.toThrow();
