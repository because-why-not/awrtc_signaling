...
export interface ISignalingPeer {
   ...
   */
  acceptOutgoingConnection(
    otherPeer: ISignalingPeer,
...
export class SignalingPeer {
  ...
  private mState: SignalingConnectionState =
    SignalingConnectionState.Uninitialized;
  ...
  private sendToClient(evt: NetworkEvent) {
    ...
    if (this.mState == SignalingConnectionState.Connected) {
      ...
      this.mLog.logv(
        `dropped message of type ${NetEventType[evt.Type]} due to state ${SignalingConnectionState[this.mState]}`,
      );
    }
  }
  ...
  private nextConnectionId(): ConnectionId {
    ...
    this.mNextIncomingConnectionId = new ConnectionId(
      this.mNextIncomingConnectionId.id + 1,
    );
    return result;
  }
  ...
  public acceptOutgoingConnection(
    otherPeer: ISignalingPeer,
    newConnectionId: ConnectionId,
  ) {
  ...
  public denyConnection(address: string, newConnectionId: ConnectionId) {
    ...
    this.sendToClient(
      new NetworkEvent(NetEventType.ConnectionFailed, newConnectionId, null),
    );
  }
  ...
  private disconnect(connectionId: ConnectionId) {
    ...
    if (otherPeer != null) {
      ...
      if (idOfOther === undefined) {
        this.mLog.error(
          "Tried to disconnect from a peer that is not connected. Bug in IController not creating bidrectional connections? " +
            otherPeer.getIdentity(),
        );
        return;
      }
  ...
  private listen(address: string) {
    ...
    if (this.mOwnAddress != null) this.stopListen();
  ...
  private stopListen() {
    ...
    if (this.mOwnAddress == null) {
      ...
      this.mLog.error(
        "Tried to stop listening on an address but no address was set. Server or client side bug?",
      );
      return;
    }
    this.sendToClient(
  ...
  private forwardMessage(
    ...
    if (reliable)
      this.sendToClient(
        new NetworkEvent(NetEventType.ReliableMessageReceived, id, msg),
      );
    else
      this.sendToClient(
        new NetworkEvent(NetEventType.UnreliableMessageReceived, id, msg),
      );
  }
  private sendData(id: ConnectionId, msg: Uint8Array, reliable: boolean) {
    ...
    if (peer) {
      ...
      this.mLog.info(
        "Message dropped. Tried to send message to unknown connection id " +
          id.id,
      );
    }
  }
