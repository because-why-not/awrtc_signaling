...
config.apps.forEach((app) => {
  signalingServer.addPeerPool(
    app.path,
    new DefaultPeerPool(app, logger.createSub(app.path)),
  );
});
...
if (env_port) {
  logger.log(
    "The environment variable process.env.port or PORT is set to " +
      env_port +
      ". Ports set in config json will be ignored",
  );

  //overwrite config ports to use whatever the cloud wants us to
  if (config.httpConfig) config.httpConfig.port = validatePort(env_port);

  if (config.httpsConfig) config.httpsConfig.port = validatePort(env_port);
  ...
  if (config.httpConfig && config.httpsConfig) {
    ...
    logger.warn(
      "Only http/ws will be started as only one port can be set via process.env.port.",
    );
    logger.warn(
      "Remove the httpConfig section in the config.json if you want to use https" +
...
if (config.httpConfig) {
  ...
  httpServer.listen(options, function () {
    logger.log(
      "websockets/http listening on " + JSON.stringify(httpServer.address()),
    );
  });
  ...
  httpsServer.listen(options, function () {
    logger.log(
      "secure websockets/https listening on " +
        JSON.stringify(httpsServer.address()),
    );
  });
