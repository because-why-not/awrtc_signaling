...
export abstract class PeerPool implements IPeerController {
  ...
  public removeConnection(client: ISignalingPeer) {
    ...
    if (index != -1) {
      ...
      this.mLog.warn(
        "Tried to remove unknown SignalingClientConnection. Bug?" +
          client.getIdentity(),
      );
    }
  }
   ...
   * @param address
   */
  public abstract onListeningRequest(
    client: ISignalingPeer,
    address: string,
  ): void;
   ...
   * @param address
   */
  public abstract onStopListening(
    client: ISignalingPeer,
    address: string,
  ): void;

  public abstract onConnectionRequest(
    client: ISignalingPeer,
    address: string,
    id: ConnectionId,
  ): void;
  ...
  public onConnectionRequest(
    ...
    if (listenerPeers && listenerPeers.length == 1) {
      ...
      this.mLog.warn(
        "Peer " +
          client.getIdentity() +
          " attempted to create an outgoing connection to a shared address. This indicates a client side bug.",
      );
