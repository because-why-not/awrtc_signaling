...
export class BinaryWebsocketProtocol extends Protocol {
  ...
  constructor(ep: WebsocketEndpoint, logger: ILogger) {
    ...
    this.mEndPoint = ep;
    this.mLog = logger;
    this.mEndPoint.ws.on("message", (data, isBinary) =>
    ...
    this.mPongReceived = true;
    this.mPingInterval = setInterval(() => {
      this.doPing();
    }, 30000);
  ...
  public send(evt: NetworkEvent): void {
    ...
    if (this.mEndPoint.ws.readyState == WebSocket.OPEN) {
      ...
      this.mLog.warn(
        `dropped message of type ${NetEventType[evt.Type]} because the websocket is not open. Websocket state: ${this.mEndPoint.ws.readyState}`,
      );
    }
  }
  ...
  private onMessage = (inmessage: RawData, isBinary: boolean) => {
    if (isBinary === false) {
      this.mLog.error(
        "Received a websocket message with isBinary == false but expected true!",
      );
      return;
    }
    if (Buffer.isBuffer(inmessage) == false) {
      this.mLog.error(
        "Received a websocket message event with an unexpected RawData format!",
      );
      return;
    }
    try {
  ...
  // - onClose
  private cleanup() {
    if (this.mIsDisposed) return; //already cleaned up
    ...
    if (this.mEndPoint.ws.readyState != WebSocket.CLOSED) {
      ...
      setTimeout(() => {
        if (fallbackWs.readyState != WebSocket.CLOSED) {
          this.mLog.logv(
            "Terminating websocket after " + closingTimeout + "ms.",
          );
